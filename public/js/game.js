$(window).on("load", function(){
  ...
  const opp = localStorage.getItem('opponent');
  const newOpp = JSON.parse(opp);

  $("img#users-game-celeb-image").attr("src", parse.image);
  $("div#users-game-celeb-name").text(parse.name);
  $("div#temp-health-display").text(parse.hitpoints);

  $("img#opponent-game-celeb-image").attr("src", newOpp.image);
  $("div#opponent-game-celeb-name").text(newOpp.name);
  $("div#temp-health-display2").text(newOpp.hitpoints);
...
var audioElement = document.createElement("audio");
  audioElement.setAttribute("src", "audio/heavy-punches.mp3");
...
// katyPerryQuotesArray =     "I'm capable of anything, of anything and everything, now you will bleed!",
